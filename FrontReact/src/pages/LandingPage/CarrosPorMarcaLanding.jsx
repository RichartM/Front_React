...
import CarCardLanding from './CarCardLanding';
...
const CarrosPorMarcaLanding = () => {
  const { brandId } = useParams();
  const { brands } = useContext(BrandsContext);
  const [cars, setCars] = useState([]);
  const location = useLocation();
const esLandingPublica = location.pathname.includes("/landing");
  ...
  {cars.length > 0 ? (
    cars.map((car) => (
      <CarCardLanding key={car.id} car={car} brandId={brandId} /> // ← Siempre muestra versión cliente
    ))
  ) : (
...
export default CarrosPorMarcaLanding;